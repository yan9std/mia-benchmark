...
class REAAttack:
    ...
    def _summary_path(context: AttackContext) -> str:
        return os.path.join(context.checkpoint_dir("reminisence"), "lira_rea_summary.json")

    @staticmethod
    def _curve_paths(context: AttackContext) -> tuple[str, str]:
        base = context.checkpoint_dir("reminisence")
        return (
            os.path.join(base, "lira_rea_tpr.npy"),
            os.path.join(base, "lira_rea_fpr.npy"),
        )

    @classmethod
    def run(cls, context: AttackContext, dry_run: bool = False) -> AttackResult | None:
        cmd = cls.build_command(context)
        print(f"[attack:rea] {' '.join(cmd)}")
        if dry_run:
            return None

        subprocess.run(cmd, check=True)
        result = cls.collect(context)
        summary_path = os.path.join(context.attack_result_dir("rea"), "attack_result.json")
