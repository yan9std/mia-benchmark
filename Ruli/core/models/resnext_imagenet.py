...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    ...
    if pretrained:
        ...
        new_state_dict = {k: v for k, v in state_dict.items() if
                          k in model.state_dict() and "fc" not in k and model.state_dict()[k].shape == v.shape}
...
def wrn28_10():
    return Wide_ResNet(28, 10, 0.3, 200)


class SwinTransformerModified(nn.Module):
    def __init__(self, num_classes=200):
        super(SwinTransformerModified, self).__init__()
        self.swin = timm.create_model('swin_large_patch4_window7_224', pretrained=True, num_classes=num_classes)
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        #self.head = nn.Linear(self.swin.num_features, num_classes)
        self.head = nn.Linear(self.swin.head.in_features, num_classes)

    def forward(self, x):
        x = self.swin.forward_features(x)
        x = self.avgpool(x)  # Reduce the spatial dimensions to 1x1
        x = torch.flatten(x, 1)  # Flatten the features for the classifier
        x = self.head(x)  # Final classification layer
        return x